...
func main() {
	...
	for userIds == "" {
		log.Printf("No user ids detected. Sleeping 1 minute and trying to pull new user ids.")
	...
	for {
		...
		// Check if user ids have been updated
		if i%10 == 0 {
			var newUserIds = getUserIds()
			i = 1

			for newUserIds == "" {
				log.Printf("No user ids detected. Sleeping 1 minute and trying to pull new user ids.")
				time.Sleep(1 * time.Minute)
				newUserIds = getUserIds()
			}

			if userIds == newUserIds {
				continue
			}
			...
			var v = url.Values{}
			v.Set("follow", newUserIds)
			log.Printf("Changing user ids to %s", newUserIds)
