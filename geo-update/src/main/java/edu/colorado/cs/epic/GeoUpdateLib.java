...
public class GeoUpdateLib {
			...
			if (currentBlob.getName().endsWith("json.gz")) {
				callables.add(new Callable<Integer>() {
					public Integer call() throws Exception {
						return updateTweetData(storage, srcBucketName, currentBlob.getName(), destBucketName,
								currentBlob.getName());
					}
				});
			}
	...
	private static Integer updateTweetData(Storage storage, String srcFolder, String srcFile, String destFolder,
			String destFile) {
		// Reads a source tweet file, then creates an updated version as a destination
		// file.
			...
			if (blob != null) {
				...
				try {
					...
					if (destBucket == null) {
						...
						// System.out.println("INFO: A new destination bucket " + destFolder + " has
						// been created.");
					}
	...
	private static String fixGeoTaggedTweet(String tweet) {
		if (!tweet.isEmpty()) {
			try {
				// OrderedJSONObject is an extension of the basic JSONObject, provided by Apache
				// Wink,
				// that keeps the order while parsing the String.
				// This class allows control of the serialization order of attributes.
				// The order in which items are put into the instance controls the order in
				// which they are serialized out.
		...
		if (!srcBuffer.isEmpty()) {
			...
			while ((oneByte = inStream.read()) != -1) {
				...
				if ((char) oneByte != '\n') { // oneByte == 10
					...
					// Increment tweet count
					count = count + 1;
					System.out.printf("\rTweet count: %-8d", count);
